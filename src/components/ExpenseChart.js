...
function ExpenseChart({ expenses, selectedMonth = "All", selectedYear = "All" }) {
  ...
  // Bar Chart: Monthly breakdown by category (for the selected year)
  ...
  const monthlyData = months.map((month, i) => {
    const monthExpenses = expenses.filter((exp) => {
      if (!exp.createdAt) return false;
      const d = new Date(exp.createdAt);
      const isInYear = selectedYear === "All" || d.getFullYear() === selectedYear;
      return isInYear && d.getMonth() === i;
    });

    const obj = { month };
    categories.forEach((cat) => {
      obj[cat] = monthExpenses
        .filter((e) => (e.category || "Uncategorized") === cat)
        .reduce((sum, e) => sum + e.amount, 0);
    });
    return obj;
